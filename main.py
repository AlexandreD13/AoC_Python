...
from src.days import IDay
from src.days.implementations import (
    Day1_2021,
    Day2_2021,
    Day3_2021,
    Day4_2021,
...
def main() -> None:
    ...
    for day in days:
        tests: list[str] = run_tests(day.name.capitalize())
        ...
        if len(day.data) > 0:
            ...
            print(
                f"\tTime taken:\t\t\t{Color.BLUE}{Color.BOLD}{end_time * 1000:.2f} ms{Color.END}"
            )
        else:
            print(
    ...
    print(
        f"\n{Color.GREEN}{Color.BOLD}Total Time taken:\t\t\t{total_time * 1000:.2f} ms{Color.END}\n"
    )
...
def run_real_data(day: IDay, tests: list[str]):
    ...
    print(f"\tResult of part 1:\t\t{Color.BLUE}{Color.BOLD}{part1_result:,}{Color.END}")
    print(
        f"\tResult of part 2 - Tests:\t"
        f"{Color.BLUE}{Color.BOLD}{"PASSED" if tests and "PASSED" in tests[1] else "FAILED"}{Color.END}"
    )
    print(f"\tResult of part 2:\t\t{Color.BLUE}{Color.BOLD}{part2_result:,}{Color.END}")


def run_tests(name: str) -> list[str]:
    f: StringIO = StringIO()

    with redirect_stdout(f):
        pytest.main(["-v", f"src/days/tests/{name}_2021_test.py"])
